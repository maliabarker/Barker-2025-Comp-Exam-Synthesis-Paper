...
Photometric techniques involve measuring the intensity of a star’s light (also called flux) over time. These observations are commonly made with telescopes across various wavelengths, primarily in the optical and near-infrared, which we focus on in this section.
...
The main two parameters we can derive from the RM effect are the projected rotation speed of the stellar surface ($v \sin i_*$) and the sky-projected angle between the planet's orbital path and the stellar equator, $\lambda$ \citep{gaudi2007prospects, winn2010transits}.
...
With many new space telescope missions planned for the future, along with the increasing usage of ground-based observatories and personal telescopes, the opportunity for data collection has never been greater. Both photometric and spectroscopic observations can be made on the ground and in space, but each type of observation has its advantages and challenges. We first consider differences in the accessibility of space-based vs. ground-based observatories.
...
On the other hand, space-based missions like NASA's Transiting Exoplanet Survey Satellite (TESS) continually observe different sectors of the sky. TESS collects stellar flux data from a different part of the sky about every month, making it an invaluable resource for exoplanet research \citep{guerrero2021tess}. Future ground-based missions, such as the Vera C. Rubin Observatory, will scan the sky nightly with the largest CCD camera ever built, generating massive amounts of data \citep{daukantas2024vera}. While this data will be publicly available, processing and storing such an enormous volume of information will present significant challenges, likely requiring advanced data processing pipelines.
...
To mitigate errors caused by the Earth's motion around the Sun and the Sun's motion around the solar system barycenter, a dynamical timing system is necessary to avoid cyclical variations in astronomical time series. This is why we use the Barycentric Julian Date (BJD) system with the TDB timing scale. Historically, the Heliocentric Julian Date (HJD) was more commonly used because it is easier to calculate. Although one could argue that using HJD or BJD does not significantly impact most observations—since the difference is about one second \citep{eastman2010achieving} and most transit observations do not have comparative accuracy to be affected—there is an important distinction between the two. HJD is typically based on the UTC time scale, while BJD is based on the TDB time scale, and the difference between these scales is approximately 64.184 seconds (as of 2024, \citep{adams2024doomed}), with this offset changing as leap seconds are added. For this reason, it is crucial to report both the timing system and time scale used in any data analysis, as they are not always explicitly stated and are often assumed to be associated (UTC with HJD, TDB with BJD).

Composite times, which are derived from multiple light curves, also present several challenges. The error bars on composite times are typically much smaller because of the consolidation of timing information into a single point. Smaller error bars skew model fits (see Section \ref{sec:models} for further explanation of this process), resulting in possible false detections of decreasing orbital periods. Composite times can also obscure the original data source, making it harder to identify potential timing errors and may lead to double-counting transits.

Composites are often used in cases such as stacked light curves and weighted average times, where transits from multiple observations are combined or averaged to reduce noise and improve detection. This is especially common in ground-based surveys where detecting a single transit is difficult. The consolidation of timing information into a single point results in much smaller error bars, which can skew model fits (see Section \ref{sec:models} for further explanation), potentially leading to false detections of decreasing orbital periods. Additionally, composite times can obscure the original data source, making it harder to identify potential timing errors, and may lead to double-counting transits. For instance, with CoRoT-2, 82 transits were stacked \citep{alonso2008transiting}, and when unstacked, the original transits had errors that were 50\% larger, effectively ruling out tidal decay \citep{adams2024doomed}.

Many systems initially evaluated as having significant orbital period changes often contain discrepancies in their data, necessitating critical re-analysis to ensure accuracy. To address these issues, \citet{adams2024doomed} recommends implementing a standardized timing system and scale to prevent false timing conversions, publishing light curves directly to facilitate future re-analysis, and clearly specifying the original data source if it is not newly reported in the literature. Moving forward, establishing a standardized database of timing data would help mitigate these challenges and improve the reliability of orbital period analyses.
...
As mentioned in Sections \ref{sec:intro} and \ref{sec:obs-photo}, transit and occultation light curves can tell us about planetary and stellar properties and orbital dynamics. By tracking changes in mid-times and/or orbital parameters, we can also determine the likelihood of tidal decay. Even when it is possible to have direct observations and numerical values, it can help to have analytical approximations as well as uncertainties and covariances to predict the likelihood of a future transit or even optimize future observing plans to maximize the likelihood of detecting orbital decay, for example.

% Metrics for predicting transits
Analytical approximations are good for planning observations. For example, which systems will have the most precise measurement of orbital inclination, how many transits do I need to observe before statistical error in planetary radius is smaller than systematic error. They can also help in speeding up the convergence of optimization algorithms, and can be useful in order-of-magnitude estimates of observability of subtle transit effects such as transit-timing variations (TTVs), precession-induced changes, and asymmetry in ingress and egress due to non-zero eccentricity \citep{carter2008analytic}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{figs/carter_fig1.png}
    \caption{Figure 1 from \citet{carter2008analytic} show the piecewise linear model (solid) and exact source model (dashed) of a transit light curve. The vertical axis represents normalized flux,
    where $f_0$ denotes the constant stellar flux, and $f_0-\delta$ represents the maximum flux reduction due to the planet blocking the star. The horizontal axis marks the four contact times, as described in Figure \ref{fig:winnfig2}, with the transit mid-time denoted as $t_c$. The fractional transit depth is given by $\delta$, T approximates the total transit duration, and $\tau$ represents the ingress/egress duration.}
    \label{fig:carter-fig1}
\end{figure}

The models presented in \citet{carter2008analytic} assume flux measurements are made continuously throughout the transit, stellar limb darkening is negligible, the orbital period is large compared to the transit duration, and the transit occurs in a uniform, straight line motion across the stellar disk. An illustration of this model compared to the exact source model is given by \ref{fig:carter-fig1}, which shows that the deviation from the exact source model is small and therefore, this approximation can accurately be made.
...
%     \caption{Figure 3 from \citet{winn2010transits} illustrates the geometry of a transit and the likelihood of the transit being seen (can tie into the analytical models from Carter or just use it to explain why we would want an idea of the likelihood (because it may not happen based on complicated orbital properties)).}
%     \label{fig:winn-fig3}
% \end{figure}

% Metrics for predicting tidal decay
% Describing delta BIC
Furthermore, we can use models to help analyze timing data. The decrease in orbital period for UHJs is very small, along the scale of milliseconds for Earth year, and is therefore hard to detect. This makes unguided searches for tidal decay pretty inefficient, especially when it sometimes takes a few years to detect a significant decay signal. Metrics provided by \citet{jackson2023metrics} help us guide these searches. The linear and quadratic models and BIC metric help us find the statistical significance of tidal decay in preexisting data so we can determine which systems are most likely to exhibit tidal decay (and therefore which systems we should focus on). The BIC metric can also help us project the likelihood of detecting significant signals of tidal decay for future observations based on frequency and total number of observations.

To find tidal decay, we can fit a set of midtime observations, called an ephemeris, to a linear model (which assumes no change in orbital period), a quadratic model (which assumes a rate of orbital change), and an apsidal precession model (which assumes a sinosouidal rate of orbital change). We then determine which model best represents the data. Each orbit of an exoplanet is denoted by an epoch number, and each midtime is associated with an orbit number, or epoch. 

An ephemeris linear in orbital epoch $E$ has a predicted mid-transit and mid-occultation time given by:

\begin{equation}
    t_{\text{tra}}^{\text{pred}} = T_0 + PE
\end{equation}

\begin{equation}
    t_{\text{occ}}^{\text{pred}} = T_0 + \frac{P}{2} + PE
\end{equation}

An ephemeris quadratic in orbital epoch $E$ has a predicted mid-transit and mid-occultation time given by:

\begin{equation}
    t_{\text{tra}}^{\text{pred}} = T_0 + PE + \frac{1}{2}\frac{dP}{dE}E^2 
\end{equation}

\begin{equation}
    t_{\text{occ}}^{\text{pred}} = T_0 + \frac{P}{2} + PE + \frac{1}{2}\frac{dP}{dE}E^2 
\end{equation}

And an ephemeris sinouisoidal in orbital epoch $E$ has a predicted mid-transit and mid-occultation time given by:

\begin{equation}
    t_{\text{tra}}^{\text{pred}} = T_0 + P_sE - \frac{eP_a}{\pi} \cos\omega(E)
\end{equation}

\begin{equation}
    t_{\text{occ}}^{\text{pred}} = T_0 + \frac{P_a}{2} + P_sE + \frac{eP_a}{\pi} \cos\omega(E)
\end{equation}

where $T_0$ is the midtime at $E = 0$, $P$ is the orbital period. Similar ephemeris models apply for mid-occultation times. Timing data that better fits a linear model favors NO tidal decay while timing data that better fits a quadratic model favors tidal decay.
...
From \citet{adams2024doomed}, delta BIC values below 30-50 should be tentative because of usual timing data errors, especially in the estimation of timing uncertainties

The correction of unrealistic timing uncertainties is significant in confirming or ruling out tidal decay for systems.
