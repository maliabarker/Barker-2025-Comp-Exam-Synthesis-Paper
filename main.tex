...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{figs/ogilvie_fig1.png}
...
% Figure \ref{fig:ogilvie-fig1} illustrates the interaction between a UHJ and its host star, where the star is treated as a monopole and the planet as a quadrupole. The tidal bulge raised on the planet due to the star’s gravity is misaligned with the axis connecting the two bodies, leading to tidal forces that transfer angular momentum and dissipate energy. Over time, these forces can drive the system toward tidal locking, where the planet’s rotation period synchronizes with its orbital period, causing one hemisphere to always face the host star.
...
\begin{figure}[htbp]
    ...
    \caption{Tidal forcing frequencies for quadrupolar tidal components listed in Table \ref{fig:ogilvie-tbl1}. 
    The vertical axis describes the tidal frequency in the fluid frame, which is the angular frequency measured in frame of reference rotating with the body ($\hat{\omega}$), divided by the body's spin frequency ($\Omega_s$). This tells us how the tidal forces compare to the body's own revolution about its axis. 
    The horizontal axis is the orbital frequency ($\Omega_o$), which is the rate that body 1 rotates around body 2, divided by the spin frequency ($\Omega_s$), which is the rate that body 1 spins about its axis. This tells us how fast the body is orbiting compared to its spin. The dashed vertical line at $\frac{\Omega_o}{\Omega_s} = 1$ indicates alignment of the orbital and spin frequencies (e.g., tidal locking). 
    Each colored line, labeled by a set of quadrupolar components from Table \ref{fig:ogilvie-tbl1}, has slope $n$ and y-intercept $-m$.
...
\begin{figure}[htbp]
    ...
    \caption{The tidal evolution of a planet-star system (assuming a circularized orbit with $e=0$). The vertical axis shows the stellar spin angular velocity (in units of the critical angular velocity, $\Omega_c$, which is associated with the critical angular momentum, $L_c$). The horizontal axis shows the planetary orbital angular velocity (also in units of critical angular velocity $\Omega_c$). The solid lines are contours of the total angular momentum of the system $L$, with arrows pointing in the direction of evolution such that energy is dissipated. The dashed line represents when the stellar spin and planetary orbit are synchronized. Black points are moments of tidal equilibrium, when $L \geq L_c$.}
    \label{fig:ogilvie-fig10}
\end{figure}

Figure \ref{fig:ogilvie-fig10} shows different trajectories of a planet tidally interacting with its host star, based on the total angular momentum of the system in terms of the critical angular momentum ($\frac{L}{L_c}$), assuming the orbit is circularized such as in a UHJ system. 

If the system's angular momentum $L$ is greater than or equal to the critical angular momentum, $L_c$, then the system can stay in equilibrium for a long time. Points of equilibrium are represented by the black dots in Figure \ref{fig:ogilvie-fig10}. The system tends to move towards tidal equilibrium (black dots) and synchronization (dashed line), as seen by the directional arrows on the contour lines. 

Above the synchronization line, there is more spin angular momentum, and the star spins down as the system attempts to dissipate energy. Below this line, there is more orbital angular momentum, and tidal equilibrium is not reached ($L < L_c$). In this scenario, the system continuously transfers angular momentum from the orbit to the spin, resulting in a shrinking orbit period and stellar spin-up. This is tidal decay.

<some transition into talking about energy dissipation and tidal quality factor>

% Q* explanation 

The tidal quality factor $Q_*$ is a dimensionless, non-literal parameterization of the tidal response that represents how efficiently the system dissipates tidal energy \citep{ogilvie2014tidal}. While $Q_*$ is commonly used in theoretical settings, it is hard to directly measure. Instead, a more commonly used metric is the modified tidal quality factor $Q_*' = \frac{Q_*}{k_2}$ \citep{patel2022constraining}, where $Q_*$ is the tidal quality factor and $k_2$ is the Love number, which measures how a body deforms due to a tidal response. The modified tidal quality factor accounts for the deformation of a body, making it easier to measure from observations of tidal effects. A larger value of $Q'_*$ means weaker dissipation, and a smaller value of $Q'_*$ means stronger dissipation.

The value of $Q'_*$ depends on the tidal frequency and the different quadrupolar components of the spherical harmonics $l$ and $m$, meaning the value of $Q'_*$ differs with internal structure of the body \citep{ogilvie2014tidal}. For example, when looking at binary star recircularization, we find $Q^{'}_{*} \approx 10^6$ \citep{ogilvie2007tidal}. However, in UHJ systems, this value can vary greatly, with WASP-12 having $Q^{'}_{*} = 1.8 \times 10^5$ \citep{yee2019orbit}.
...
Many statistical models have previously assumed a constant value of $Q^{'}_{*}$ (see Section \ref{sec:models} for an in-depth review of these models and assumptions). However, this may decrease model accuracy significantly, as the value of $Q^{'}_{*}$ depends intricately on stellar age, mass, and internal structure. For example, in Table \ref{}, \citet{adams2024doomed} show the observed change in orbital period and the theoretically calculated change in orbital period if the value of $Q'_*$ is assumed to be equal to WASP-12 b. In the four systems shown, we see drastic differences, showing that it is not feasible to predict a constant value for $Q'_*$. 

However, one big issue with this is that it is fairly difficult to measure the value of $Q'_*$ without also making other assumptions in the observations. One way to mitigate this issue is to attempt to model the internal structure and interactions within host stars and predict the value of $Q'_*$ in this way. \citet{weinberg2023orbital} created non-linear models to develop an interpolable grid of $Q'_*$ values based on stellar parameters such as mass and age. Using this grid and system parameters, \citet{adams2024doomed} found theoretical values for a handful of extrasolar systems and compared them to $Q'_*$ values based on observations. These results are plotted in Figure \ref{fig:adams-fig11}.

For example, in Figure \ref{fig:adams-fig11}, \citet{adams2024doomed} plot the values of $log_{10}Q^{'}_{*}$ depending on actual observed parameters (seen by the blue circles) and theorized models created by \citet{weinberg2023orbital} (seen by the purple plus signs).

The approximation of $Q'_*$ from \citet{goldreich1966q}:

\begin{equation}
    Q'_* = - \left( \frac{27 \pi}{2} \right ) \left( \frac{dP}{dt} \right )^{-1} \left( \frac{M_p}{M_s} \right ) \left( \frac{a}{R_*} \right )^{-5}
\end{equation}

shows that $Q'_*$ is inversely proportional to the rate of orbital period change \citep{adams2024doomed}. Using this approximation, \citet{adams2024doomed} calculated the value of $Q'_*$ based on the observed orbital period rate of change, and the theoretical value of $Q'_*$ based on the 
