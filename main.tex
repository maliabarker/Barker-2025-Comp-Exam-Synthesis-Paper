...
- just regular spectroscopy:
    - can find "Changes in the stellar absorption lines can be produced by absorption features in the planetary atmosphere" (learn more about the atmosphere), (gaudi and winn)

- rossiter-mclaughlin effect 
    - can find the projected rotation speed of the stellar surface
    - can find $\lambda$, the angle between sky projections of the stellar spin axis and the orbit normal (provides info about planetary spin-orbit alignment, which can then give us info about the synchronization due to tidal forces mentioned by Ogilvie, or even "possible means of discriminating among migration theories" (gaudi and winn)) (QUESTION: is this the same as the lambda in winn 2010)

- planet doesn't need to transit BUT transit spectroscopy is super valuable
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{figs/winn_fig5.png}
    \caption{Figure 5 from \citep{winn2010transits} illustrates the Rossiter-McLaughlin effect, which uses spectroscopy, or the collected of light wavelengths, to detect an exoplanet transit.}
    \label{fig:winn-fig5}
\end{figure}

The Rossiter–McLaughlin effect, derived from papers on eclipsing binary stars by \citep{rossiter1924detection} and \citep{mclaughlin1924some}, and first observed in exoplanets by \citep{queloz2000detection} and \citep{bundy2000search} is based on the Doppler effect, which is the shift in frequency of a wave as an object moves towards or away from an observer. For example, as an emergency vehicle approaches an observer, they may hear the pitch of the sirens increase, then hear a decrease in pitch as it retreats. This occurs because as the vehicle quickly approaches, the sound waves in front of it are compressed, resulting in a higher frequency and, therefore, a higher pitch. As the vehicle retreats, the sound waves are stretched, resulting in a lower frequency and, therefore, a lower pitch. 
...
\begin{figure}
    ...
    \caption{Figure 1 from \citep{triaud2017rossiter} illustrates the Rossiter-McLaughlin effect for the hot Jupiter HD 189733A b. CONTINUE EXPLAINING STUFF}
    \label{fig:triaud-fig1}
\end{figure}

When an object as large as a star rotates, the hemisphere moving toward the observer is blueshifted, and the hemisphere moving away from the observer is redshifted, as seen in Figure \ref{fig:triaud-fig1}. As a planet transits the star and blocks out starlight, we can observe an average flux offset shifted redward if the planet is covering the blueshifted hemisphere, and blueward if the planet is covering the redshifted hemisphere. 

DIRECT QUOTE: The distribution of λ (or rather, of |λ|) with other (planetary or stellar) parameters can also be used to study the exchange of angular momentum between the planet and its host star, via tidal forces (pull the info from winn paper to show this)
