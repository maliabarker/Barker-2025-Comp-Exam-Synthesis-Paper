...
% - orbital evolution of hot jupiters (we should expect to see them falling into their stars, especially because UHJs tend to exist around younger stars, so what is happening to them?) 
% - observations and data collection of hot jupiters (transits & transit light curves)
...
Changes in the orbital period may not always indicate tidal decay. For example, if an exoplanet has an elongated orbit indicative of a non-zero eccentricity and the orbital plane is also moving 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figs/adams_fig2.png}
    \caption{$\Delta$ BIC values for 43 exoplanets. For all panels, a positive $\Delta$ BIC value favors a quadratic model over a linear model, suggesting a change in the orbital period. Blue bars indicate negative $\dot{P}$, meaning a decreasing orbital period, and red bars indicate a positive $\dot{P}$, indicating an increasing orbital period. Two grey lines are plotted with the dashed line at $\Delta$ BIC equal to 30 and the dotted line at $\Delta$ BIC equal to 10. The top plot shows the full range of $\Delta$ BIC values with WASP-12 b obviously surpassing all other planets. The middle panel shows the same information as above, zoomed in to show the smaller $\Delta$ BIC values. The five systems with the highest values of $\Delta$ BIC after WASP-12 b have their uncertainties on P??? plotted below the corresponding bar. The bottom panel shows the $\Delta$ BIC values after the data is rescaled.}
    \label{fig:enter-label}
\end{figure}

% Describing the rescaling process
In reported literature, light curves often do not have realistic uncertainties on the mid-times. When we fit a model to transit data, we add weights on each point using the photometric uncertainty. If a specific data point has a very small value for their uncertainty, it can influence the model to fit that data point more than the others, essentially "weighing" down the resulting fit. Small error bars can result in a very impactful data point.
...
When fitting a model, we determine the accuracy of the model fit to the actual data using the Bayesian Information Criterion (BIC) \citet{schwarz1978estimating}

\begin{equation}
    \text{BIC} = \chi^{2} + k\text{ln}N
\end{equation}

where $k$ is the number of fit parameters (two for a linear model, three for a quadratic model, and five for a precession model), $N$ is the number of data points or observations used to fit the model, and $\chi^{2}$ is the reduced chi-squared, which is the measurement of the goodness of fit, calculated by the sum of the residuals squared \citet{pearson1900criterion}:
