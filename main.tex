...
\begin{figure}[htbp]
    ...
    \caption{$\Delta$ BIC values for 43 exoplanets. For all panels, a positive $\Delta$ BIC value favors a quadratic model over a linear model, suggesting a change in the orbital period. Blue bars indicate negative $\dot{P}$, meaning a decreasing orbital period, and red bars indicate a positive $\dot{P}$, indicating an increasing orbital period. Two grey lines are plotted with the dashed line at $\Delta$ BIC equal to 30 and the dotted line at $\Delta$ BIC equal to 10. The top plot shows the full range of $\Delta$ BIC values with WASP-12 b obviously surpassing all other planets. The middle panel shows the same information as above, but zoomed in to show the smaller $\Delta$ BIC values. The five systems with the highest values of $\Delta$ BIC have their The bottom panel shows the $\Delta$ BIC values rescaled }
...
where $k$ is the number of fit parameters (two for a linear model, three for a quadratic model, and five for a precession model), $N$ is the number of data points or observations used to fit the model, and $\chi^{2}$ is the measurement of the goodness of fit, calculated by the sum of the residuals squared \citet{pearson1900criterion}:

\begin{equation}
    \chi^{2} = \sum\frac{(O_i - E_i)^2}{E_i}
\end{equation}

where $O_i$ is the observed value, or actual data point, and $E_i$ is the expected value calculated by the model.

Larger values of $\chi^2$ 

To minimize bias for more complex models, we use BIC, which penalizes additional model parameters by adding $k$ to the final BIC value.
